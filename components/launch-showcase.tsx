...
export function LaunchShowcase() {
  ...
  return (
    <section className="page-section page-wrap">
      <SectionHeading
        eyebrow={examplesOnly ? "Product examples" : "Public launches"}
        title={examplesOnly ? "See how a community launch could select an official reward." : "Community launches with official Fan Token rewards."}
        copy={examplesOnly ? "These examples explain the product. They are not tokens, markets, holders, fees, or funded rewards." : "Published SportPad records appear here without invented market figures."}
        action={<Link href="/discover" className="text-link">View all <ArrowRight /></Link>}
      />
      {feedLoading ? <div className="examples-notice"><span>Loading public launches.</span></div> : null}
