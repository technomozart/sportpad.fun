...
test("Chiliz treasury key accepts only 64 hex characters with an optional 0x prefix", () => {
  const hex = "aB".repeat(32);
  assert.equal(normalizeChilizTreasuryPrivateKey(hex), `0x${hex}`);
  assert.equal(normalizeChilizTreasuryPrivateKey(` \n0x${hex}\t`), `0x${hex}`);
  for (const invalid of [
    ...
    `${hex}0`,
    `${hex.slice(0, -1)}g`,
    `${hex.slice(0, 32)} ${hex.slice(32)}`,
  ]) {
    assert.throws(() => normalizeChilizTreasuryPrivateKey(invalid), /CHILIZ_TREASURY_PRIVATE_KEY must contain 64 hexadecimal characters/);
  }
});

test("Chiliz worker requires the configured public address to match its derived account", () => {
  const address = getAddress("0x1234567890abcdef1234567890abcdef12345678");
  const badChecksum = address.replace(/[a-fA-F]/, (letter) =>
    letter === letter.toLowerCase() ? letter.toUpperCase() : letter.toLowerCase());
  assert.equal(assertChilizTreasuryAddressMatchesAccount(address, address), address);
  assert.equal(assertChilizTreasuryAddressMatchesAccount(` ${address.toLowerCase()} `, address), address);
  assert.throws(() => assertChilizTreasuryAddressMatchesAccount(badChecksum, address), /CHILIZ_TREASURY_ADDRESS is invalid/);
  assert.throws(() => assertChilizTreasuryAddressMatchesAccount("", address), /CHILIZ_TREASURY_ADDRESS is invalid/);
  assert.throws(() => assertChilizTreasuryAddressMatchesAccount("0x1234", address), /CHILIZ_TREASURY_ADDRESS is invalid/);
  assert.throws(() => assertChilizTreasuryAddressMatchesAccount("0x0000000000000000000000000000000000000000", address), /does not match/);
});
