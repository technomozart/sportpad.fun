...
export function normalizeChilizTreasuryPrivateKey(value) {
  const key = typeof value === "string" ? value.trim() : "";
  if (!/^(?:0x)?[0-9a-fA-F]{64}$/.test(key)) {
    throw new Error("CHILIZ_TREASURY_PRIVATE_KEY must contain 64 hexadecimal characters, optionally prefixed by 0x.");
  }
  return key.startsWith("0x") ? key : `0x${key}`;
}

export function assertChilizTreasuryAddressMatchesAccount(value, accountAddress) {
  const configuredAddress = typeof value === "string" ? value.trim() : "";
  if (!isAddress(configuredAddress)) throw new Error("CHILIZ_TREASURY_ADDRESS is invalid.");
  const checksumAddress = getAddress(configuredAddress);
  if (checksumAddress !== getAddress(accountAddress)) {
    throw new Error("CHILIZ_TREASURY_ADDRESS does not match the worker account.");
  }
