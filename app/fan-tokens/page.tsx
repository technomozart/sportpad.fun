  ...
  return (
    <SiteChrome>
      <main className="page-wrap inner-page">
        <PageIntro kicker="Official Fan Token markets" title="Explore 78 Kayen-listed Chiliz markets and two Solana options." copy="A published address or market listing does not prove that SportPad has acquired reward inventory. Quotes are checked at selection and again before a future mainnet launch; payout execution remains paused until the workers and treasuries are verified.">
          <div className="verification-seal"><BadgeCheck /><span><strong>{chilizAssets.length} + {solanaAssets.length}</strong> listed market options</span><small>Quote checks do not fund claims</small></div>
        </PageIntro>
        <SafetyNotice>These are official Fan Tokens issued for the named sports organizations, not new SportPad copies. When Chiliz payouts are enabled and funded, a verified claim would unwrap Kayen&apos;s wrapped trading token and send the original Fan Token to the user&apos;s linked 0x wallet. No such payout is active now.</SafetyNotice>
        ...
        <section className="content-section">
          <div className="registry-stats"><div><strong>{chilizAssets.length}</strong><span>Kayen-listed markets</span></div><div><strong>{solanaAssets.length}</strong><span>Solana market options</span></div><div><strong>88888</strong><span>Chiliz Chain ID</span></div><div><strong>Paused</strong><span>reward payout execution</span></div></div>
        ...
        <section className="page-section network-explainer">
          <div><p className="section-eyebrow">Two proposed claim networks</p><h2>Each funded reward would be claimed on its own network.</h2><p>A Solana claim would use the verified Solana wallet. A Chiliz claim requires a separately verified 0x address; the dashboard can ask MetaMask to add or switch to Chiliz Chain. The planned worker would pay CHZ claim gas and unwrap funded Kayen inventory. This payout lane is not yet active.</p><Button asChild variant="outline" className="mt-6 rounded-full border-white/10 bg-transparent text-white hover:bg-white/8 hover:text-white"><Link href="/how-it-works">See the full route <ArrowRight /></Link></Button></div>
          <div className="network-compare"><div><span>SOLANA</span><strong>Two listed options</strong><ul><li>Verified Base58 wallet</li><li>SPL Fan Token</li><li>Jupiter market quote</li><li>Claim only after funding</li></ul></div><div><span>CHILIZ CHAIN</span><strong>78 listed markets</strong><ul><li>Verified 0x wallet</li><li>Official Fan Token</li><li>Kayen market quote</li><li>CHZ gas sponsorship planned</li></ul></div></div>
        </section>
