...
export function LaunchBuilder() {
  ...
  const selected = useMemo(() => fanAssets.find((asset) => asset.symbol === reward), [reward]);
  const filteredAssets = useMemo(() => {
    const query = rewardQuery.trim().toLowerCase();
    return fanAssets.filter((asset) => !query || `${asset.name} ${asset.symbol} ${asset.category}`.toLowerCase().includes(query));
  }, [rewardQuery]);
  const canContinue = step === 0 ? name.trim().length >= 2 && symbol.length >= 2 && Boolean(imageFile) && !imageValidating : step === 1 ? Boolean(selected) && rewardRoute.state === "available" : step === 2 ? Object.values(terms).every(Boolean) : true;
  ...
  useEffect(() => {
    ...
    fetch(`/api/reward-routes/${encodeURIComponent(reward)}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
    ...
    if (step === 1) setValidationMessage(
      !selected
        ...
        : rewardRoute.state === "checking"
          ? "Wait for the live Jupiter route check to finish."
          : rewardRoute.state === "unavailable"
            ? `${selected.symbol} has no live SOL acquisition route on Jupiter right now. Choose a routed reward.`
            : "The live Jupiter route could not be verified. Retry or choose another reward.",
    );
  ...
  return (
    <>
      ...
      <div className="launch-form-panel">
        ...
        {step === 1 ? <div className="form-step"><div className="form-title"><BadgeCheck /><span><h3>Choose the official Fan Token holders may earn.</h3><p>Every option is registry-listed by Chiliz. SportPad checks the selected Solana mint against Jupiter before it can be paired as a reward.</p></span></div><label className="reward-search"><Search /><span className="sr-only">Search official Fan Tokens</span><Input value={rewardQuery} onChange={(event)=>setRewardQuery(event.target.value)} placeholder={`Search ${fanAssets.length} official Fan Tokens`} /></label><div className="reward-selector">{filteredAssets.map((asset)=><button key={asset.symbol} type="button" aria-pressed={reward === asset.symbol} className={reward === asset.symbol ? "selected" : ""} onClick={()=>{ setRewardRoute({ state: "checking" }); setReward(asset.symbol); setRewardRouteRequest((value)=>value+1); setRewardNotice(""); }}><TokenMark token={asset.symbol} color={asset.color} imagePath={asset.imagePath} size="lg" /><span><strong>{asset.name}</strong><small>${asset.symbol} · {asset.category}</small><code>{asset.mint.slice(0,7)}...{asset.mint.slice(-6)}</code></span><span className="asset-status status-registry-listed">Official on Solana</span>{reward === asset.symbol ? <CheckCircle2 className="selected-check" /> : null}</button>)}</div>{filteredAssets.length === 0 ? <div className="form-info"><Info /><p>No official Fan Token matches that search.</p></div> : null}{rewardNotice ? <div className="form-info" role="status"><Info /><p>{rewardNotice}</p></div> : null}{selected ? <div className={`form-info reward-route-${rewardRoute.state}`} role="status"><Info /><p><strong>{selected.name} is an official Fan Token rooted in the Chiliz ecosystem.</strong> {rewardRoute.state === "checking" ? "Checking its live SOL route on Jupiter." : rewardRoute.state === "available" ? "A live SOL acquisition route is available on Jupiter." : rewardRoute.state === "unavailable" ? "No live SOL acquisition route is available on Jupiter right now, so SportPad will not accept this pairing." : rewardRoute.state === "error" ? "The route provider could not be reached. Retry by selecting the token again." : "Select the token to run a live route check."}</p></div> : <div className="form-info"><Info /><p>Select one official Fan Token deliberately. SportPad will not choose a default reward for you.</p></div>}</div> : null}

        {step === 2 && selected ? <div className="form-step"><div className="form-title"><LockKeyhole /><span><h3>Review the immutable mainnet route.</h3><p>The creator receives 0% of this creator fee stream.</p></span></div><div className="economics-cards"><div><Trophy /><span><small>80%</small><strong>Official Fan Token rewards</strong><p>Creator fees fund {selected.symbol} acquisition and time-weighted holder reward epochs.</p></span></div><div><Flame /><span><small>20%</small><strong>SPORTPAD buyback + burn</strong><p>Proceeds accrue in the buyback treasury until the SPORTPAD mint and audited burn executor are active.</p></span></div></div><div className="rules-table"><div><span>Pump market pair</span><strong>SOL</strong></div><div><span>Reward pairing</span><strong>{selected.symbol} via creator fees</strong></div><div><span>Reward route</span><strong className="positive">Jupiter checked</strong></div><div><span>Unsafe acquisition</span><strong>Pause, never force</strong></div><div><span>Pump holder rewards</span><strong>Disabled</strong></div><div><span>Fee split</span><strong>One-time 80 / 20 lock</strong></div></div><div className="terms-list">{[{key:"rights",label:"I have the right to use the submitted name, copy, and image."},{key:"unofficial",label:"I understand the community token and official Fan Token reward are separate assets."},{key:"economics",label:"I understand the 80/20 creator-fee route is designed to be irreversible."}].map((item)=><label key={item.key}><input type="checkbox" checked={terms[item.key as keyof typeof terms]} onChange={(event)=>setTerms((current)=>({...current,[item.key]:event.target.checked}))}/><span>{terms[item.key as keyof typeof terms] ? <Check /> : null}</span>{item.label}</label>)}</div></div> : null}

        {step === 3 && selected ? <div className="form-step"><div className="form-title"><Goal /><span><h3>One last check before saving.</h3><p>This saves a private project draft. The creator wallet signs mainnet transactions only after content approval.</p></span></div><div className="review-card"><div className="review-identity">{imagePreview ? <img className="review-artwork" src={imagePreview} alt={`${name} token image`} /> : <TokenMark token={symbol || "SP"} color="#9cff57" size="lg" />}<span><strong>{name}</strong><small>${symbol} · Community token draft</small></span></div><dl><div><dt>Description</dt><dd>{description || "No description provided"}</dd></div><div><dt>Reward asset</dt><dd>{selected.name} (${selected.symbol}) official Fan Token</dd></div><div><dt>Solana token address</dt><dd><code>{selected.mint}</code></dd></div><div><dt>Reward route</dt><dd className="positive">Live Jupiter route verified</dd></div><div><dt>Economics</dt><dd>80% Fan Token rewards · 20% SPORTPAD buyback + burn</dd></div><div><dt>Execution</dt><dd>Mainnet after content approval and wallet signatures</dd></div></dl></div>{error ? <p className="form-error" role="alert">{error}</p> : null}</div> : null}
