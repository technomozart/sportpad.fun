      ...
      <main className="page-wrap inner-page token-page">
        ...
        <section className="token-hero">
          ...
          <div className="token-reward-card">
            ...
            <p>The official {rewardAsset.symbol} Fan Token is the selected reward asset in this {launch.isExample ? "example" : isMainnet ? "mainnet launch" : "published devnet receipt"}. It is separate from the Solana community coin. A funded claim would pay on {rewardAsset.chain === "chiliz" ? "Chiliz Chain" : "Solana"}; selection alone does not create a claim.</p>
            <dl><div><dt>{rewardAsset.chain === "chiliz" ? "Chiliz contract" : "Solana token address"}</dt><dd><code>{rewardTokenAddress}</code></dd></div><div><dt>Identity</dt><dd>{rewardAsset.status}</dd></div><div><dt>Market venue</dt><dd>{isMainnet ? `${rewardAsset.route} quote checked at launch` : rewardAsset.route}</dd></div><div><dt>Reward funding</dt><dd>{isMainnet ? "80% fee recipient published; inventory not implied" : "Not funded by this example or devnet receipt"}</dd></div></dl>
          </div>
        </section>
        ...
        <section className="page-section">
          ...
          <div className="four-checks">
            ...
            <article><span>04</span><Flame /><h3>20% buyback treasury</h3><p>{isMainnet ? "The immutable fee configuration sends 20% to the SPORTPAD buyback treasury. A verified purchase and burn are separate steps." : "The proposed configuration reserves 20% for a future SPORTPAD buyback and burn; neither occurs in this example or devnet receipt."}</p></article>
          </div>
        </section>
        ...
        <section className="page-section contract-grid">
          ...
          <div><p className="section-eyebrow">Official reward asset</p><h3>{rewardAsset.name}</h3><dl><dt>Symbol</dt><dd>{rewardAsset.symbol}</dd><dt>Network</dt><dd>{rewardAsset.chain === "chiliz" ? "Chiliz Chain" : "Solana"}</dd><dt>Verification</dt><dd><BadgeCheck /> Official contract match</dd><dt>Market venue</dt><dd>{isMainnet ? `${rewardAsset.route} quote checked at launch` : rewardAsset.route}</dd><dt>Reward funding</dt><dd>{isMainnet ? "80% fee recipient published; inventory not implied" : "Not funded by this receipt"}</dd></dl></div>
          <div className="contract-risk"><ShieldCheck /><h3>{isMainnet ? "Verified identity and fee routing." : "Verified identity, disabled execution."}</h3><p>{isMainnet ? "The official Fan Token contract, a point-in-time market quote, community mint, and 80/20 Pump fee recipients are public. A quote and fee split do not prove funded inventory or a claim; those require separate receipts." : "The official Fan Token contract is known. Acquisition, custody, allocation, and claims are not enabled for this receipt."}</p></div>
        </section>
