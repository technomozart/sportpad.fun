...
export function SystemStatus() {
  ...
  return (
    <div className="status-grid" aria-live="polite">
      ...
      <div><span className={health?.mainnetExecution ? "healthy" : "warning"} /><strong>Mainnet launcher</strong><small>{health?.mainnetExecution ? "Reward and buyback preflight verified" : `Launches paused: ${health?.mainnetMissing.join(", ") || "configuration unavailable"}`}</small><code>{health?.mainnetExecution ? "ENABLED" : "PAUSED"}</code></div>
      ...
      <div><span className={protocol?.automation.chilizActive || protocol?.automation.solanaActive ? "healthy" : "warning"} /><strong>Fan Token payouts</strong><small>{protocol ? `${protocol.counts.confirmedClaims} confirmed payouts` : "Loading payout state"}</small><code>{protocol?.automation.chilizActive || protocol?.automation.solanaActive ? "HEARTBEAT" : "PAUSED"}</code></div>
      ...
      <div><span className={protocol?.automation.chilizActive ? "healthy" : "neutral"} /><strong>Chiliz reward lane</strong><small>{protocol?.automation.chilizActive ? "Chiliz worker heartbeat observed; funding and payouts require separate verification" : "Chiliz treasury worker not observed"}</small><code>{protocol?.automation.chilizActive ? "HEARTBEAT" : "OFF"}</code></div>
    </div>
  );
}
