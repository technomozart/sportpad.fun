...
export function SystemStatus() {
  ...
  return (
    <div className="status-grid" aria-live="polite">
      ...
      <div><span className={protocol?.capabilities.walletExecutionEnabled ? "healthy" : "warning"} /><strong>Fan Token payouts</strong><small>{protocol ? `${protocol.counts.confirmedClaims} confirmed payouts` : "Loading payout state"}</small><code>{protocol?.capabilities.walletExecutionEnabled ? "WALLET READY" : "PAUSED"}</code></div>
      ...
      <div><span className={protocol?.capabilities.signerProviderConfigured ? "healthy" : "neutral"} /><strong>Unattended automation</strong><small>{protocol?.capabilities.signerProviderConfigured ? "Managed signer provider configured" : "Treasury transactions remain wallet-confirmed"}</small><code>{protocol?.capabilities.signerProviderConfigured ? "CONFIGURED" : "OFF"}</code></div>
