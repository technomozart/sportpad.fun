    ...
    <SiteChrome>
      <main className="page-wrap inner-page">
        ...
        <section id="networks" className="learn-article">
          <div className="article-index">02</div><div><p className="section-eyebrow">Wallets and networks</p><h2>Solana identifies the holder. Chiliz receives Chiliz rewards.</h2><p>Chiliz Chain is its own EVM-compatible network, not Ethereum mainnet. Current Fan Tokens can also have official representations on Solana and Base through an omnichain token architecture. Each network still has a different address, native gas token, and local token standard.</p><div className="network-facts"><div><strong>SOLANA</strong><span>Base58 address · SPL Token · SOL gas</span><small>Community coin holdings are verified here; Solana reward claims use this wallet</small></div><div><strong>CHILIZ</strong><span>0x address · CAP-20 · CHZ gas</span><small>Chiliz claims require a separately verified MetaMask or compatible 0x wallet</small></div></div><p>A Solana reward would need only the verified Solana wallet. A Chiliz reward also requires a verified 0x address. The dashboard can request the Chiliz network in MetaMask, but wallet linking does not itself create a claim or fund a reward. Chiliz payouts are currently paused.</p></div>
        </section>
