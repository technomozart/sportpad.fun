...
const glossary = [
  ...
  ["Verified reward asset", "A current chain-specific Fan Token address matched to a first-party registry. Identity alone does not verify a purchase or payout route."],
    ...
    <SiteChrome>
      <main className="page-wrap inner-page">
        ...
        <section id="networks" className="learn-article">
          <div className="article-index">02</div><div><p className="section-eyebrow">Wallets and networks</p><h2>Solana identifies the holder. Chiliz would receive Chiliz rewards.</h2><p>Chiliz Chain is its own EVM-compatible network, not Ethereum mainnet. Chiliz moved Fan Tokens to new 18-decimal V2 contracts in 2026; older zero-decimal contracts and their Kayen wrappers are not the current reward assets. Some Fan Tokens also have official representations on Solana and Base. Each network has a different address, gas token, and token standard.</p><div className="network-facts"><div><strong>SOLANA</strong><span>Base58 address · SPL Token · SOL gas</span><small>Community coin holdings are verified here; Solana reward claims would use this wallet</small></div><div><strong>CHILIZ</strong><span>0x address · CAP-20 · CHZ gas</span><small>Chiliz claims would require a separately verified MetaMask or compatible 0x wallet</small></div></div><p>A Solana reward would need only the verified Solana wallet. A Chiliz reward would also require a verified 0x address and an audited route to the current V2 contract. The dashboard can request the Chiliz network in MetaMask, but wallet linking does not create a claim or fund a reward. Chiliz payouts are paused.</p></div>
        </section>
        ...
        <section id="inventory" className="learn-article"><div className="article-index">04</div><div><p className="section-eyebrow">Liquidity and inventory</p><h2>A published token address does not mean market ready.</h2><p>A current Fan Token contract can have little local supply or no safe executable route. The 2026 Chiliz V2 migration also means a quote for an old Kayen wrapper is not a quote for the current Fan Token. Before enabling any reward, SportPad must verify the exact V2 or Solana asset, quote depth, price impact, inventory coverage, token accounts, and claim delivery.</p><div className="state-definitions"><span><b className="healthy" /> <strong>Ready</strong><small>Would require verification, funding, and open claims</small></span><span><b className="warning" /> <strong>Limited</strong><small>Would reserve existing claims and cap new capacity</small></span><span><b className="neutral" /> <strong>Replenishing</strong><small>Would wait for destination funding</small></span><span><b className="danger" /> <strong>Paused</strong><small>Would indicate a safety or infrastructure stop</small></span></div></div></section>

        <section id="proof" className="learn-article"><div className="article-index">05</div><div><p className="section-eyebrow">Verify, don’t trust</p><h2>Every number needs an authoritative source.</h2><p>Current token identity must come from the official V2 migration table or Solana registry. Balances and supply must come from finalized blockchain state. Executable prices must come from live quotes for that exact contract at the intended size. Future rewards must be traceable through fee, purchase, vault, epoch, and payout transactions.</p><div className="source-links"><a href="https://docs.chiliz.com/learn/about-fan-tokens/2026-migration-to-decimal-fan-tokens" target="_blank" rel="noreferrer">Official Chiliz V2 contracts <ExternalLink /></a><a href="https://solscan.io" target="_blank" rel="noreferrer">Solana explorer <ExternalLink /></a><Link href="/transparency">SportPad transparency explorer <ArrowRight /></Link></div></div></section>

        <section id="risk" className="learn-article"><div className="article-index">06</div><div><p className="section-eyebrow">Risk disclosure</p><h2>Rewards do not protect a bad trade.</h2><p>Community coins and Fan Tokens are volatile and can lose all value. Quotes can disappear, liquidity can become shallow, swaps can fail, bridges can delay, smart contracts can contain bugs, and third-party utility can change.</p><ul className="risk-list"><li>Never trade solely because rewards are advertised.</li><li>Check the current V2 contract, not an older Chiliz token or wrapper address.</li><li>Verify the coin, reward token address, route state, and exact transaction before signing.</li><li>A Solana-held Fan Token may not unlock Socios.com polls, staking, or experiences.</li><li>SportPad will never ask for a seed phrase or private key.</li><li>Transactions are irreversible; Solana and 0x addresses are not interchangeable.</li><li>Rewards are not interest, dividends, equity, APR, or guaranteed income.</li></ul></div></section>
        ...
        <section className="final-cta compact-cta"><div><p className="section-eyebrow">Ready to explore?</p><h2>See the ideas applied to clearly labeled examples.</h2><p>Examples distinguish a community coin from the current official reward contract; no example has a funded reward or enabled Chiliz route.</p></div><div><Button asChild className="h-12 rounded-full bg-[#9cff57] px-6 text-[#071008] hover:bg-[#adff7d]"><Link href="/discover">Explore examples <ArrowRight /></Link></Button><Link href="/how-it-works" className="text-link">Planned protocol mechanics <Landmark /></Link></div></section>
      </main>
    </SiteChrome>
  );
}
